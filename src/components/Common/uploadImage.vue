...
export default {
  ...
  methods: {
    ...
    handleAvatarSuccess(res, file) {
      if (this.limitHeight && this.limitWidth && res.height && res.width) {
        if (this.imageRatio && (res.width / res.height > this.imageRatio || res.width / res.height < this.imageRatioMin)) {
      ...
      if (this.rowScope) {
        ...
        if (this.keyImage) {
          rowScope[this.keyImage] = res.url
        }
        if (res.width && res.height) {
          rowScope[this.keyRatio] = (res.width / res.height).toFixed(1)
        }
        this.$emit('input', { ...this.rowScope.row, ...rowScope })
        this.$emit('change', { ...this.rowScope.row, ...rowScope })
        return
