...
export default {
  ...
  props: {
    ...
    childsLoadApi: {
      type: String,
      default: function() {
        return this.$route.path.replace('list', 'childs?id={id}')
      }
    },
    rowChangeApi: {
      type: String,
  ...
  methods: {
    ...
    getList: function(filterData) {
      ...
      request({
        ...
        const payload = res
        const payloadListSize = payload.items.length
        if (this.infiniteScrollMode) {
          this.tableData = [...this.tableData, ...payload.items]
          this.noMoreData = !payload.has_more
        } else {
          if (payloadListSize > 20) {
            this.tableData = payload.items.slice(0, 20)
            setTimeout(() => {
              payload.items.slice(20).forEach((item) => {
        ...
        this.page.total = payload.total_result
    ...
    getQueryDataWithExtra: function() {
      let extra = { page: this.page.currentPage, limit: this.page.pageSize }
    ...
    rowChange: function({ row, key, val }) {
      const data = {}
      data[key] = val
      console.log(this.$props.options.rowChangeApi)
      request({
        // todo
        ...
        this.$message.success(res._message || '保存成功')
      })
    },
