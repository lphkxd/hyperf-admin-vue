...
export default {
  ...
  methods: {
    ...
    sendRequest(data) {
      ...
      const uris = this.realTarget.split(':')
      let url = this.realTarget
      let method = this._.toUpper(this.$props.method || 'POST')
      if (uris.length === 2) {
        url = uris[1]
        method = uris[0]
      }
      request({
        ...
        this.loading = false
        this.popoverShow = false
        this.showMessage(res.messageRule, res._message || '操作成功')
        setTimeout(function() {
