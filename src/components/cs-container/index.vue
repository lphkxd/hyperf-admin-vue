<template>
  <div class="container-component" :class="{responsive}" ref="container">
    <!-- [card] 卡片容器 -->
    <el-card v-if="type === 'card'" shadow="never" class="cs-container-card cs-mr">
      <slot v-if="$slots.header" name="header" slot="header"/>
      <slot/>
    </el-card>
    <!-- [ghost] 隐形的容器 -->
    <div v-if="type === 'ghost'" class="cs-container-ghost">
      <el-card v-if="$slots.header" shadow="never" class="cs-container-ghost-header">
        <slot name="header"/>
      </el-card>
      <slot/>
    </div>
    <!-- [container-full] 填充 -->
    <cs-container-full v-if="type === 'full' && !scroll">
      <slot v-if="$slots.header" name="header" slot="header"/>
      <slot/>
      <slot v-if="$slots.footer" name="footer" slot="footer"/>
    </cs-container-full>
    <!-- [container-full-bs] 填充 滚动优化 -->
    <cs-container-full-bs v-if="type === 'full' && scroll">
      <slot v-if="$slots.header" name="header" slot="header"/>
      <slot/>
      <slot v-if="$slots.footer" name="footer" slot="footer"/>
