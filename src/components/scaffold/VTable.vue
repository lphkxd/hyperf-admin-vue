...
export default {
  ...
  computed: {
    filter: function() {
      return Object.assign(this.form.formData(), { page: this.page.currentPage, limit: this.page.pageSize })
    },
    emptyText: function() {
  ...
  methods: {
    ...
    getList: function(initFilter) {
      ...
      request({
        ...
        this.page.total = res.total_result
    ...
    selectRemote(field, keyWord) {
      ...
      request({
        ...
        if (res.length > 0) {
          this.form.model()[field].options = res
        }
