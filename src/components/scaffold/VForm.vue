  ...
  methods: {
    ...
    formRule(force_request = false) {
      ...
      request({
        ...
        this.formMaker(data)
        this.loading = false
        this.realComputeMap = res.compute_map
        this.formOptions = this._.merge(this.formOptions, res.form_ui || {})
      }).catch(() => {
        this.loading = false
      })
    },
    ...
    onSubmit: _.throttle(function(formData) {
      ...
      request({
        ...
        Message({
          message: (res._message || '操作成功').replace(/\n/g, '<br/>'),
          type: 'success',
          duration: 5 * 1000,
    ...
    selectRemote(field, keyWord, row) {
      ...
      request({
        ...
        const ret = JSON.stringify(res) === '{}' ? [] : res
