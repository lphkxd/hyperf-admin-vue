...
service.interceptors.response.use(
  ...
  error => {
      ...
      default:
        if (res.code === 40012) {
          MessageBox.confirm(res.message, '提示', {
            ...
            config.data = JSON.stringify(Object.assign(JSON.parse(config.data), {
              _repeat: true
            }))
            return backoff.then(() => {
    ...
    if (!config.shouldRetry || typeof config.shouldRetry !== 'function') return Promise.reject(error)
    ...
    // 重新发起axios请求
    return backoff.then(() => {
      return service(Object.assign({}, config, {
        'baseURL': ''
      }))
    })
  }
